        ...
        if (listingsGrid) {
          if (snapshot.empty) {
            ...
            snapshot.forEach((docSnap) => {
              ...
              const badgeClass =
                data.status === "pending" ? "pending-badge" : "active-badge";
              const statusTxt =
                data.status === "pending" ? "Pending" : "Active";
              ...
              <div class="listing-card" onclick="window.location.href='/marketplace.html'">
                ...
                <div class="listing-body">
                  ...
                  <div class="listing-meta">
                    <span class="listing-price">${data.price ? data.price + " FCFA" : "Free"}</span>
      ...
      (snapshot) => {
        ...
        if (!snapshot.empty) {
          ...
          snapshot.forEach((docSnap) => {
            ...
            const pAva =
              c.partnerAvatar ||
              `https://ui-avatars.com/api/?name=${encodeURIComponent(pName)}&background=635bff&color=fff`;
...
onAuthStateChanged(auth, (user) => {
  ...
  if (!hasSeenSplash) {
    ...
    if (welcomeEl && user.displayName)
      welcomeEl.textContent = `WELCOME BACK, ${user.displayName.split(" ")[0].toUpperCase()}!`;
  }

  const heroGreeting = document.getElementById("heroGreeting");
  if (heroGreeting && user.displayName)
    heroGreeting.textContent = `Good Morning, ${user.displayName.split(" ")[0]}!`;
